...
export class Authentication {
    ...
    public static generatePasswordHashFromSalt(password: string, salt: string) {
        let size: number = parseInt(process.env.STREAME_HASH_SIZE, 10); // 32
        let keyMultiplier: number = parseInt(process.env.STREAME_HASH_MULTIPLIER, 10); // 4
        let iterations: number = parseInt(process.env.STREAME_HASH_ITERATIONS, 10); // 1024
        let digest: string = process.env.STREAME_HASH_DIGEST; // 'sha512'
